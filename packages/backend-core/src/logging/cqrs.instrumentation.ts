...
export class CqrsInstrumentation implements OnModuleInit, OnApplicationBootstrap {
  ...
  private wrapEventBusPublish(bus: EventBus) {
    ...
    bus.publish = <T extends AbstractDomainEvent<any, any, any, any>>(event: T) => {
      this.logEventPublish(event);
      return originalPublish(event);
    };

    bus.publishAll = <T extends AbstractDomainEvent<any, any, any, any>>(events: T[]) => {
      events.forEach((event) => this.logEventPublish(event));
      return originalPublishAll(events);
    };
  }

  private logEventPublish(event: AbstractDomainEvent<any, any, any, any>) {
  ...
  private wrapEventHandlers() {
    ...
    for (const wrapper of providers) {
      ...
      metatype.prototype.handle = async function (event: AbstractDomainEvent<any, any, any, any>) {
